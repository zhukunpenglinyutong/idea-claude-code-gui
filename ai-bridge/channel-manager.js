 ...
 *   mimo     - MiMo Code CLI (OpenCode fork; spawns local `mimo` binary)
 *
 * Commands:
...
import { handleMimoCommand } from './channels/mimo-channel.js';
...
(async () => {
  ...
  try {
    ...
    if (!provider || !providerHandlers[provider]) {
      console.error('Invalid provider. Use "claude", "codex", "grok", "kimi", "opencode", "pi", "omp", "dsh", "minimax", "mimo", or "system"');
      writeJsonAndExit({
        success: false,
